(function(){
  ...
  function parseDateKey(k){
    ...
    if(!m)return null;
    const d=new Date(Number(m[1]),Number(m[2])-1,Number(m[3]));d.setHours(0,0,0,0);
    return Number.isNaN(d.getTime())?null:d;
  }
  function nextDateKey(k){const d=parseDateKey(k);if(!d)return'';d.setDate(d.getDate()+1);return localDateKey(d)}
  ...
  function processMode(mode,todayKey,created){
    ...
    Object.keys(bucket).sort().forEach(sourceKey=>{
      ...
      postponed.forEach(todo=>{
        ...
        target.push(nextTodo);created.push({mode,date:targetKey,todo:nextTodo});changed=true;
      });
    });
    return changed;
  }
  ...
  function processPostponedTodos(force=false){
    ...
    try{
      changed=processMode('job',todayKey,created)||changed;
      changed=processMode('work',todayKey,created)||changed;
      lastProcessedStamp=stamp;
      if(changed){
        created.forEach(x=>{
          if(typeof window.persistTodoItemToVault==='function')window.persistTodoItemToVault(x.mode,x.date,x.todo);
          else if(typeof window.todoMainAckEnqueueUpsert==='function')window.todoMainAckEnqueueUpsert(x.mode,x.date,x.todo);
        });
  ...
  if(previousRenderAll){renderAll=function(){processPostponedTodos(false);return previousRenderAll()}}
  function refreshAfterCheck(force=true){setTimeout(()=>{if(processPostponedTodos(force)&&typeof renderAll==='function')renderAll()},300)}
  document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='visible')refreshAfterCheck(true)});
  ...
  setInterval(()=>{if(document.visibilityState==='visible'&&processPostponedTodos(false)&&typeof renderAll==='function')renderAll()},30000);
  setTimeout(()=>{if(processPostponedTodos(true)&&typeof renderAll==='function')renderAll()},1400);
})();
